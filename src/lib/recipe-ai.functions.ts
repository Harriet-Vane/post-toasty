...
export const generateAiRecipe = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<AiRecipeResult> => {
    ...
    const system = [
      ...
      "Write 4 to 7 numbered steps that reference the actual bread and toppings the user chose. Each step is one short sentence under ~90 characters, starting with a plain verb (Toast, Spread, Add, Top, Finish). Keep the language simple — most steps are just the action ('Toast the sourdough.' 'Spread the peanut butter.'). Only add a 'why' clause (to loosen it, for a crisp crust, etc.) when the reason is genuinely non-obvious to an experienced toast-maker; skip it otherwise. Most steps should have no reason clause at all. A specific temperature, time, or thickness is fine without explanation.",
